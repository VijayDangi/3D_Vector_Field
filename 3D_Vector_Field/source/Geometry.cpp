...
)
{
    ...
    //casting
    glm::vec3 *positionVector = (glm::vec3 *)position;
    glm::vec2 *textureVector = (glm::vec2 *)texcoord;

    glm::vec3 *tangentVector = (glm::vec3 *)outTangent;
    glm::vec3 *biTangentVector = (glm::vec3 *)outBitangent;

    ZeroMemory(tangentVector, sizeof(glm::vec3) * vertices_count);
    ZeroMemory(biTangentVector, sizeof(glm::vec3) * vertices_count);
    ...
    {
        tangentVector[i] = glm::normalize(tangentVector[i]);
        biTangentVector[i] = glm::normalize(biTangentVector[i]);
    }
...
bool CreateSphereGeometry( Geometry* geometry, int slices, int stacks, float radius)
{
    ...
    for( i = 0; i < stacks; i++)
    {
        ...
        for( j = 0; j < slices; j++)
        {
            ...
            normals[3*indexV + 0] = y;
            normals[3*indexV + 1] = x;
    ...
    glBindVertexArray( geometry->vao);
        geometry->vbo_position  = CreateArrayBuffer( positions, 3 * size, GL_FLOAT, 3, EVertexAttributeIndex::Position);
        geometry->vbo_normal    = CreateArrayBuffer(   normals, 3 * size, GL_FLOAT, 3, EVertexAttributeIndex::Normal);
        geometry->vbo_texcoord  = CreateArrayBuffer(  textures, 2 * size, GL_FLOAT, 2, EVertexAttributeIndex::Texcoord);
        geometry->vbo_tangent   = CreateArrayBuffer(  tangents, 3 * size, GL_FLOAT, 3, EVertexAttributeIndex::Tangent);
        geometry->vbo_bitangent = CreateArrayBuffer(biTangents, 3 * size, GL_FLOAT, 3, EVertexAttributeIndex::Bitangent);
